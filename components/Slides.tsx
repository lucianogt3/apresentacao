...
export const SlideCover: React.FC<SlideProps> = () => (
  <div className="flex flex-col justify-center h-full gap-6 md:gap-8 my-auto print:block print:pt-10">
    <div>
    ...
    <div className="flex flex-wrap gap-4 mt-4 md:mt-8 print:mt-12">
      <div className="flex items-center gap-3 px-4 py-2 md:px-6 md:py-3 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-300 font-semibold backdrop-blur-sm text-sm md:text-base print:border-black print:text-black print:bg-transparent">
        <Calendar size={18} />
        <span>01/10/2025 a 30/11/2025</span>
      </div>
      <div className="flex items-center gap-3 px-4 py-2 md:px-6 md:py-3 rounded-full bg-slate-800 border border-slate-700 text-slate-300 font-medium text-sm md:text-base print:border-black print:text-black print:bg-transparent">
        <User size={18} />
        <span>Resp: Enf. Jeiza da Silva Santos</span>
      </div>
    </div>

    <div className="mt-8 md:mt-12 p-4 md:p-6 rounded-2xl bg-slate-800/50 border border-slate-700/50 print:border-black print:bg-transparent print:mt-12">
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mt-4">
        {KPIS.map((kpi, idx) => {
          ...
          return (
            <div key={idx} className="bg-slate-800/50 p-4 md:p-6 rounded-2xl border border-slate-700 hover:border-blue-500/50 transition-all shadow-lg print:border-black print:shadow-none print:bg-transparent print:rounded-lg">
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {REVENUE_LEAKS.map((item, idx) => {
          ...
          return (
            <div key={idx} className="group bg-gradient-to-br from-slate-800 to-slate-900 p-4 md:p-6 rounded-2xl border border-slate-700 relative overflow-hidden print:border-black print:bg-none print:bg-transparent">
      ...
      <div className="mt-6 md:mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        <div className="bg-slate-800/30 p-4 md:p-6 rounded-xl border border-slate-700 print:border-black print:bg-transparent">
        ...
        <ChartContainer isPrinting={isPrinting} height={400}>
          <BarChart data={WEEKLY_FLOW_DATA} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
            ...
            <YAxis 
              ...
              tickLine={false} 
            />
            {!isPrinting && <Tooltip content={<CustomTooltip />} cursor={{fill: 'rgba(255,255,255,0.05)'}} />}
            <Legend wrapperStyle={{ paddingTop: '20px', color: colors.text }} />
            <Bar 
              dataKey="Auditoria" 
              fill={colors.barAud} 
              radius={[4, 4, 0, 0]} 
              name="Entrada (Auditoria)" 
          ...
          <div className="flex-1 print:flex-none">
            <ChartContainer isPrinting={isPrinting} height={300}>
              <LineChart data={WEEKLY_FLOW_DATA} margin={{top: 10, right: 10, left: -20, bottom: 0}}>
                ...
                {!isPrinting && <Tooltip content={<CustomTooltip />} />}
                <Line 
                  type="monotone" 
                  dataKey="Auditoria" 
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 items-center h-full print:block">
            ...
            <BarChart data={ERROR_BY_SECTOR} layout="vertical" margin={{left: 0, right: 20}}>
              ...
              <Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={40} isAnimationActive={!isPrinting}>
                {ERROR_BY_SECTOR.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={isPrinting ? (index % 2 === 0 ? '#000000' : '#888888') : entry.fill as string} />
                ))}
              </Bar>
            </BarChart>
          </ChartContainer>
        ...
        <div className="space-y-3 md:space-y-4 print:space-y-2">
          ...
          <div className="bg-slate-800 p-3 md:p-4 rounded-lg border-l-4 border-green-500 print:bg-transparent print:border print:border-black">
          ...
          <div className="bg-slate-800 p-3 md:p-4 rounded-lg border-l-4 border-orange-500 print:bg-transparent print:border print:border-black">
              ...
              <tr key={item.pos} className="border-b border-slate-800/50 hover:bg-slate-800/50 transition-colors print:border-black">
                ...
                <td className="p-2 md:p-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap
                    ${item.tagType === 'danger' ? 'bg-red-900/30 text-red-400 border border-red-900/50 print:border-black print:text-black print:bg-transparent' : 
                      item.tagType === 'warning' ? 'bg-amber-900/30 text-amber-400 border border-amber-900/50 print:border-black print:text-black print:bg-transparent' : 
                      'bg-blue-900/30 text-blue-400 border border-blue-900/50 print:border-black print:text-black print:bg-transparent'}`}>
                    {item.tag}
                  </span>
                </td>
              </tr>
...
const ConvCard: React.FC<ConvCardProps> = ({ name, errors, pct, logoUrl, placeholder, colorIdx }) => {
  ...
  return (
    <div className="bg-slate-800 p-4 md:p-6 rounded-2xl border border-slate-700 flex flex-col items-center justify-center text-center gap-3 hover:border-blue-500 transition-colors group h-full print:border-black print:bg-transparent print:p-2">
...
export const SlideInsurance: React.FC<SlideProps> = () => (
  <>
    ...
    <div className="mt-6 md:mt-8 p-4 bg-slate-800/30 border border-slate-700 rounded-xl print:border-black print:bg-transparent">
       <p className="text-sm md:text-base text-slate-300 print:text-black">
...
export const SlideProfessionals: React.FC<SlideProps> = () => (
  <>
              ...
              <tr key={p.pos} className="border-b border-slate-800/50 print:border-black">
                ...
                <td className="p-3 md:p-4">
                   <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                    ${p.tagType === 'info' ? 'bg-blue-900/30 text-blue-400 print:text-black print:border print:border-black print:bg-transparent' : 'bg-amber-900/30 text-amber-400 print:text-black print:border print:border-black print:bg-transparent'}`}>
                    {p.tag}
                  </span>
                </td>
              </tr>
    ...
    <div className="mt-4 flex items-start gap-2 text-xs md:text-sm text-slate-400 bg-slate-800 p-3 rounded-lg print:border print:border-black print:bg-transparent">
      <AlertTriangle size={16} className="mt-0.5 text-yellow-500 shrink-0 print:text-black" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 items-center print:block">
        ...
        <div>
          <ul className="space-y-4 md:space-y-6">
            <li className="relative pl-4 md:pl-6 border-l-2 border-blue-500 print:border-black">
            ...
            <li className="relative pl-4 md:pl-6 border-l-2 border-orange-500 print:border-black">
            ...
            <li className="relative pl-4 md:pl-6 border-l-2 border-green-500 print:border-black">
...
export const SlideStrategy: React.FC<SlideProps> = () => (
  <div className="h-full flex flex-col justify-center print:block">
    <SectionTitle 
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mt-4 md:mt-6">
      <div className="bg-slate-800 p-6 md:p-8 rounded-3xl border border-slate-700 shadow-xl print:border-black print:shadow-none print:bg-transparent">
      ...
      <div className="bg-gradient-to-br from-blue-900/40 to-slate-900 p-6 md:p-8 rounded-3xl border border-blue-500/30 shadow-xl relative overflow-hidden print:border-black print:bg-none print:shadow-none print:bg-transparent">
...
export const SlideNextSteps: React.FC<SlideProps> = () => (
  <div className="h-full flex flex-col justify-center print:block">
    ...
    <div className="space-y-4 md:space-y-6 mt-4">
      <div className="flex gap-4 md:gap-6 items-start p-4 md:p-6 bg-slate-800/50 rounded-2xl border border-slate-700 hover:bg-slate-800 transition-colors print:border-black print:bg-transparent">
        <div className="bg-blue-500/20 p-3 md:p-4 rounded-xl text-blue-400 shrink-0 print:border print:border-black print:bg-transparent print:text-black">
      ...
      <div className="flex gap-4 md:gap-6 items-start p-4 md:p-6 bg-slate-800/50 rounded-2xl border border-slate-700 hover:bg-slate-800 transition-colors print:border-black print:bg-transparent">
        <div className="bg-emerald-500/20 p-3 md:p-4 rounded-xl text-emerald-400 shrink-0 print:border print:border-black print:bg-transparent print:text-black">
      ...
      <div className="flex gap-4 md:gap-6 items-start p-4 md:p-6 bg-slate-800/50 rounded-2xl border border-slate-700 hover:bg-slate-800 transition-colors print:border-black print:bg-transparent">
        <div className="bg-purple-500/20 p-3 md:p-4 rounded-xl text-purple-400 shrink-0 print:border print:border-black print:bg-transparent print:text-black">
...
export const SlideClosing: React.FC<SlideProps> = () => (
  <div className="flex flex-col items-center justify-center h-full text-center space-y-6 md:space-y-8 print:block print:pt-20">
    <div className="w-20 h-20 md:w-24 md:h-24 bg-blue-600 rounded-full flex items-center justify-center shadow-lg shadow-blue-900/50 mb-2 md:mb-4 animate-bounce print:border print:border-black print:bg-transparent print:text-black print:shadow-none print:mx-auto">
    ...
    <p className="text-lg md:text-xl text-slate-400 max-w-3xl leading-relaxed px-4 print:text-black mx-auto">
    ...
    <div className="mt-8 md:mt-12 pt-8 md:pt-12 border-t border-slate-800 w-full max-w-2xl print:border-black mx-auto">
